...
bool AD5933_Class::getGainFactorsSweep(double cResistance, int avgNum, double *gainFactorArr, double *pShiftArr) {
	...
	// Check control register before getting gain factors
	if (setCtrMode(STAND_BY, ctrReg) == false)
	{
		return false;
	}
...
bool AD5933_Class::getGainFactorsSweep2(double cResistance, int avgNum) {
	int ctrReg = getByte(0x80);
  ...
  // Variables declarations...
	...
	while ( (getStatusReg() & 0x04) != 0x04) // Loop while if the entire sweep in not complete
	{
		...
		// Do Serial print gainfactors and pshift to users as you proceed...
		double dummy_gF = (tSumMag / avgNum) * cResistance;
		Serial.println(dummy_gF);
		double dummy_pShift = tSumPhase / avgNum;
		Serial.println(dummy_pShift);
...
/*
 *  New Function for performing full sweep with multi-frequency gain factor. Directly print at user's device
 */

bool AD5933_Class::getComplexSweep2() {
   int ctrReg = getByte(0x80);
  ...
  while ( (getStatusReg() & 0x04) != 0x04) // Loop while if the entire sweep in not complete
  {
    ...
    // Do Serial print as you go...
    // Eqn: imp = Gainfactor[i]/magsq
    //		phi = phVal - pShift[i]
    double magSq = getMag(rComp, iComp);
    Serial.println(magSq);
    double phVal = atan2(iComp, rComp);
